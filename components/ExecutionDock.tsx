...
export default function ExecutionDock() {
  ...
  return (
    <div className="bg-[#121318] border border-[#232530] p-4 rounded-xl text-xs font-mono">
      <div className="flex justify-between items-center mb-3">
        <h3 className="text-[#e1e1e6] font-bold text-sm tracking-wider uppercase flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          [Painel 3] Execution Dock & Kill-Switch
        </h3>
        {message && (
          <span className={`px-2 py-1 rounded ${message.includes('Erro') ? 'bg-red-500/20 text-red-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
            {message}
          </span>
        )}
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-8 gap-3">
        ...
        <div className="col-span-8 mt-1">
          ...
          >
            {loading ? 'A Registar na Nuvem...' : 'Executar & Guardar Trade'}
          </button>
        </div>
      </form>
    </div>
